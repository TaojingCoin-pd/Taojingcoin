...
uint256 hashGenesisBlock("0x1214ace11e3ef2068a3029f5733aae71234378bb2c564ca21e3c6e162003b40f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >>11); // Tjcoin: starting difficulty
...
const string strMessageMagic = "Tjcoin Signed Message:\n";
...
{
    // Tjcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    if (!ReadFromDiskSync(pindex->GetBlockPos()))
        return false;
    if (GetHash() != pindex->GetBlockHash())
        return error("CBlock::ReadFromDisk() : GetHash() doesn't match index");
    return true;
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Tjcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Tjcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Tjcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Tjcoin: This fixes an issue where a 51% attack can change difficulty at will.
        ...
        do {
            ...
            if (pindexTest->pprev == NULL || pindexTest->pnext != NULL) {
                ...
                BOOST_FOREACH(CBlockIndex *pindexSwitch, vAttach) {
                    boost::this_thread::interruption_point();
                    try {
                        if (!SetBestChainSync(state, pindexSwitch))
                            return false;
                    } catch(std::runtime_error &e) {
                        return state.Abort(_("System error: ") + e.what());
                    }
...
{
   // Check it again in case a previous version let a bad block in
    if (!CheckBlockSync(state, !fJustCheck, !fJustCheck))
        return false;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Tjcoin: Special short-term limits to avoid 10,000 BDB lock limit:
    ...
    printf("comome check  proof of work start*************");
    // Check proof of work matches claimed amount
...
bool CBlock::AcceptBlockSync(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    try {
        ...
        if (dbp == NULL)
            if (!WriteToDisk(blockPos))
                return state.Abort(_("Failed to write block"));
        if (!AddToBlockIndexSync(state, blockPos))
            return error("AcceptBlock() : AddToBlockIndex failed");
     } catch(std::runtime_error &e) {
        return state.Abort(_("System error: ") + e.what());
    }
...
{
    // Tjcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    printf("common first check begin-----------\n");
    // Preliminary checks
    if (!pblock->CheckBlock(state))
        return error("ProcessBlock() : CheckBlock FAILED");
    printf(" common first check end at----------- %15"PRI64d"ms\n", GetTimeMillis() - nStart);
    CBlockIndex* pcheckpoint = Checkpoints::GetLastCheckpoint(mapBlockIndex);
...
bool ProcessBlockSync(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    // Store to disk
    if (!pblock->AcceptBlockSync(state, dbp))
        return error("ProcessBlock() : AcceptBlock FAILED");
    ...
    for (unsigned int i = 0; i < vWorkQueue.size(); i++)
    {
             ...
             ++mi)
        {
            ...
            // Use a dummy CValidationState so someone can't setup nodes to counter-DoS based on orphan resolution (that is, feeding people an invalid block based on LegitBlockX in order to get anyone relaying LegitBlockX banned)
            CValidationState stateDummy;
            if (pblockOrphan->AcceptBlockSync(stateDummy))
                vWorkQueue.push_back(pblockOrphan->GetHash());
            mapOrphanBlocks.erase(pblockOrphan->GetHash());
            delete pblockOrphan;
        }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1388628000;
        block.nBits    = 0x1f1fffff;
        block.nNonce   = 1810;
        //bits
                        #if 0
                        ...
                        //nNonce
                                ...
                                loop
                                {
                                    bcrypt(BEGIN(block.nVersion), BEGIN(thash));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Tjcoin: increase each by adding 2 to bitcoin's value.
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "block" && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        CValidationState state;
        if (ProcessBlockSync(state, pfrom, &block) || state.CorruptionPossible())
            mapAlreadyAskedFor.erase(inv);
        int nDoS = 0;
        if (state.IsInvalid(nDoS))
            if (nDoS > 0)
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TjcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TjcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TjcoinMiner(CWallet *pwallet)
{
    printf("TjcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Tjcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TjcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        loop
        {
            ...
            loop
            {
                ...
                bcrypt(BEGIN(pblock->nVersion), BEGIN(thash));
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TjcoinMiner, pwallet));
}
